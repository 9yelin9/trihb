...
def ShowLog(path_data):
    ...
    cum_e = [e/i for i, e in zip(itr, np.cumsum(energy))]
    ...
    ax.axhline(cum_e[-1], c='r', label=r'$\langle E \rangle/N=%f$' % (energy[-1]))
    cax.axhline(cum_e[-1], c='r')

    ax.plot(itr/itv, cum_e)
    cax.plot(itr[-unit_cax:], cum_e[-unit_cax:])

    cax.grid(True)
    cax.set_xticks([itr[-unit_cax], itr[-1]])
    cax.set_yticks([cum_e[-unit_cax], cum_e[-1]])
    cax.tick_params(axis='both', which='major', labelsize='x-small')
...
def ShowLat(path_data):
    ...
    ax.set_xlabel('$x$')
    ax.set_ylabel('$y$')
    ax.set_title(r'$m_z=%.4f$ $\rho_s=%.4f$ $O^{zz}=%.4f$' % (mz, rho, ozz), fontsize='small')
...
def ShowObs(dir_data_list):
    ...
    for i, dir_data in enumerate(dir_data_list):
        ...
        ax[0].plot(df['T'], df['rho'], ms=10, marker=mk_list[i], ls=ls_list[i], fillstyle='none', label=label)
        ax[1].plot(df['T'], df['ozz'], ms=10, marker=mk_list[i], ls=ls_list[i], fillstyle='none', label=label)
        print('\n'+label, df, sep='\n')
    print()

    for i, (obs, title, xpad) in enumerate(zip([r'$\rho_{s}$', r'$O^{zz}$'], ['(a)', '(b)'], [-0.19, -0.17])):
        ax[i].grid(True)
        ax[i].set_xlabel(r'$T/J$')
        ax[i].set_ylabel(obs)
        ax[i].set_title(title, ha='left', pad=8, x=xpad)
    ax[0].legend(['theta-phi', 'phi-z'], fontsize='x-small')
...
#ShowObs('data/L12_M14_eqs1e+05_mcs1e+06_tp/res/:data/L12_M14_eqs1e+05_mcs1e+06/res/')
#ShowObs('data/L12_M14_eqs1e+05_mcs1e+06/res/:data/L18_M14_eqs1e+05_mcs1e+06/res/:data/L24_M14_eqs1e+05_mcs1e+06/res/')
